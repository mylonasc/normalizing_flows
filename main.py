...
nflows = 5
width_st = 200
act_dict = {'relu' : tf.nn.relu, 'tanh' : tf.nn.tanh,'sigmoid' : tf.nn.sigmoid}
activation_st = 'tanh'

nvp = nvp_stack(nflows, x_in, width_st = width_st, n_interm = 0, st_activation = act_dict[activation_st])
...
opt = tf.train.AdamOptimizer(learning_rate = 0.01).minimize( - tf.reduce_mean(nvplp,axis = 0))
losses = [];
...
for k in range(1,200):
    ...
    if k%10 == 0 or k == 0:
        ...
        pplot.plot(dats[:,0],dats[:,1],'.', alpha = 0.5); 
        pplot.xlim([-1.5,2.5]) ; 
        pplot.ylim([-1,1]); 
